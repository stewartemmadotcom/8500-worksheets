...
>First, we made the sotu metadata into a dataframe. Then, we read the text to a variable. Next, we arranged the data by president and combined the metadata with the texts into one dataframe called sotu_whole. Then, we tokenized the words and removed stop words. Tokenization of words and removing stop words is essential to topic modeling because we want each word to stand alone for analysis and we do not want to analyze frequent, less significant words like "the". 
...
> Modern presidents speeches are meant to be broadcast on TV and are more theatrical than earlier presidents. 

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}
length_1947 <- sotu_whole %>%
  filter(year >= 1947) %>% 
  unnest_tokens(word, text) %>%
  anti_join(stop_words) %>%
  count(president, doc_id) %>%
...
> The first line counts the number of occurences for each word and then groups those counts by president. The mutate lines creates two columns; one gives the total number of words in that document and the other provides the frequency of that word by dividing it by the total number of words. Then, we arrange the dataframe in descending order and find the single top word (in term frequency) for each president. Finally, we print all of those top words to the console. 
...
```{r}
str(tidy_sotu)
unique(tidy_sotu$president)

fdr.tfidf <- tidy_sotu %>% 
  filter(president == "Franklin D. Roosevelt") %>% 
  count(doc_id, word, sort=TRUE) %>% 
  bind_tf_idf(word, doc_id, n) 
fdr.tfidf %>% arrange(desc(tf_idf))
...
```
> I chose FDR. When I used the tf-idf score, words like autocracy, projects, popular, rulers were in the top for years prior to WWII. Then, words like hitler, axis, and nurses were in the top during the war years. When I just used raw word count, I got war for most years, then government, peace, world, and forces for others. These raw word counts make sense, considering FDR was president during WWII and would have been frequently addressing the conflict in his speeches. However, we can see more nuance with the tf-idf scores about his specific concerns: hitler, the axis powers; I wonder if nurses refers to the nurses working with the military. Also, the words for his earlier years of presidency clearly focus on the New Deal relief programs. 
...
sotu_word_pairs %>% 
  ...
  ggraph(layout = "fr") + geom_edge_link(aes(edge_alpha = n, edge_width = n), edge_colour = "tomato") + geom_node_point(size = 5) + geom_node_text(aes(label = name), repel = TRUE, point.padding = unit(0.2, "lines")) + theme_void()
```
...
mb.whole <- 
  metadata %>%  
  bind_cols(mb) %>% # combine with texts
  as_tibble() # convert to tibble for better screen viewing
...
```{r}
mb.tidy <- mb.whole %>% # here, I am tidying the data by tokenizing the words and removing stop words 
  unnest_tokens(word, text) %>% 
  anti_join(stop_words)

str(mb.tidy)

mb.tidy %>% #here I calculate the raw numbers words 
  count(doc_id, word, sort = TRUE)  

mb.tfidf <- mb.tidy %>% 
  count(doc_id, word, sort = TRUE)  %>%  
  bind_tf_idf(word, doc_id, n) 
mb.tfidf %>% arrange(desc(tf_idf)) # next I calculated the tf idf scores for each word and put in descending order; it looks like there are lots of measurements (for recipes?) in these documents 
```
...
```{r}
unlink("txt", recursive = TRUE)  # Delete the txt folder
...
wc <- wc %>% arrange(desc(doc_id))
metadata <- metadata %>% arrange(desc(filename))

wc.whole <- metadata %>% 
  left_join(wc, by = c("filename" = "doc_id")) %>% 
  as_tibble()
...
wc.tfidf %>% arrange(desc(tf_idf))

wc.wordpairs <- wc.whole %>% 
  mutate(speech_end = word(text, -5000, end = -1)) %>%  # extract last 100 words
  unnest_tokens(word, speech_end) %>%   # tokenize
  filter(!word %in% stop_words$word) %>%  # remove stopwords
...
wc.wordpairs %>% 
  filter(n >= 10) %>%  # only word pairs that occur 200 or more times
  graph_from_data_frame() %>% #convert to graph
  ggraph(layout = "fr") + # place nodes according to the force-directed algorithm of Fruchterman and Reingold
  geom_edge_link(aes(edge_alpha = n, edge_width = n), edge_colour = "tomato") +
  geom_node_point(size = 5) +
...
> I think that for this data, I would need to change up the stop words. A lot of the information I am finding here is in regard to the newspaper itself, with words like subscriber being pretty prominent. However, there are also lots of words that relate to politics, like president, diplomacy, and amendment. Because this is a suffrage journal, it makes sense that there are many government oriented words. Wilson, power, workers, war, suffrage, american, and women are also very prominent. Given the historical context of WWI and Woodrow Wilson's presidency, as well as the suffrage movement's campaign for women's right to vote, all of these words make sense. Even if I didn't know what the journal was about prior to running this text analysis, the tfidf score would key me in. 