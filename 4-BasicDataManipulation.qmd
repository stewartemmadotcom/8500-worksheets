...
> I created a variable and assigned the operation to it. This way, instead of printing the result to the terminal it saves to a dataframe that can be accessed later. 
...
```{r}
gvlsc <- gayguides %>% filter(city == "Greenville" & state == "SC")
```

(@) How about every location between 1975 and 1980? 

```{r}
late.sevs <- gayguides %>% filter(Year >= 1975 & Year <= 1980)
```
...
```{r}
gvlsc <- gayguides %>% filter(Year >= 1975 & Year <= 1980 & city == "Greenville" & state == "SC")
```

(@) Can you find all locations in 1975 except for New York and San Francisco? 
```{r}
seventy.five <- gayguides %>% filter(Year == 1975 & state != "NY" & city != "San Francisco")
```
...
```{r}
women.locs <- gayguides %>% filter(grepl("(G)", amenityfeatures) | grepl("(L)", amenityfeatures))
```
...
birthyear <- BostonWomenVoters %>% mutate(birthyear = 1920 - Age)
...
```{r}
locs <- gayguides %>% mutate(location = paste(city, state, sep=", "))
```
...
```{r}
gayguides %>%
  mutate(city_standard = str_to_title(city))
```
...
```{r}
gayguides.dancing <- gayguides %>% mutate(has_dancing = grepl("(D)", amenityfeatures))
print(gayguides.dancing$has_dancing)
```

(@) In the Boston Women Voters data, occupation names may have extra whitespace. Use `str_trim()` to clean the occupation column.
```{r}
no.whitespace <- BostonWomenVoters %>% mutate(Occupation = str_trim(Occupation))
```
...
```{r}
data(statepopulations)
statepop <- statepopulations %>% select(STATE, X1850) %>% arrange(desc(X1850))
```
...
> 42 rows are missing ward information. Ward information may be missing because it was not recorded for particular voters. Maybe the ward changed over time. Regardless, this lets us know that the historical data is incomplete; not all the information was always recorded. 
...
```{r}
sum(is.na(BostonWomenVoters$Ward & BostonWomenVoters$Age))
removena <- BostonWomenVoters %>% 
  drop_na(Ward) %>% 
  drop_na(Age) 

sum(is.na(removena$Ward & removena$Age))
```
...
```{r}
ward <- BostonWomenVoters %>% mutate(ward_missing = is.na(Ward))
```
...
```{r}
factor <- gayguides %>% mutate(type_factor = as.factor(type))
factor %>% count(type_factor)
levels(factor$type_factor)
```

(@) In the Boston Women Voters data, convert Occupation to a factor. What are the levels?
```{r}
factorbwv <- BostonWomenVoters %>% mutate(occ_factor = as.factor(Occupation))
levels(factorbwv$occ_factor)
```
