...
```{r}
paste("My cat is Fiona,", "and my dogs are Jack and Bean")
```

(@) Can you explain what happened in 2-3 sentences? 

> Using the paste function, I can add two strings together. For this practice, I combined a sentence fragment about my cat with a sentence fragment about my dogs. 
...
> The first line tests if 5 is equal to 10; it is not, so it returns as FALSE. The second line tests if 3 is less than 4, which is TRUE. The third line, 5 !=10 tests if 5 is unequal to 10; TRUE. The fourth line tests if 5 is unequal to 5; FALSE. the fifth line tests if 3 is equal to 4 OR 3; TRUE. The final line tests to see if 3 is unequal to 4 and 5; TRUE. 
...
```{r}
bean.fur <- "white and spotted"
jack.fur <- "brown, black, grey, and wiry"
paste(jack.fur, bean.fur)
```
...
> In the code above, we created a vector called my.numbers that included multiple values. Then, we multiplied that vector by 2, resulting in a list of the values multiplied by 2. 
...
> Here, we specified the third value in the vector and multiplied it by two. Because the third value is 6, the result is 12. 

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
my.pets <- c("Jack", "Fiona", "Bean")
my.pets
```
...
>Floor rounds a number to the nearest digit, no deminal places, that is equal to or less than the number. 
...
> Row z uses the equation provided above, x * 2 + y, to determine z's value for each new row; x increases by 1 in each of the five rows, while y is always 1. Therefore, in row 2 (for example), z equals 2 * 2 + 1, or 5. 
...
> The documentation says that this dataset has information pulled from Bob Damron's Address Books from the years 1965-1985. The data was last updated Dec 2022. There is one data frame with 14 variables (columns) and 60698 observations (rows). 
...
> To figure out how to print the first ten rows of data rather than the first six, I looked at the documentation for the head function. There, I found that I can modify the number of rows provided by putting a number after the dataset. I also found the documentation for tail there, which prints from the last rows. 
...
>gayguides specifies the data frame we are pulling from. $ tells R to pull a specific piece of data, while title specifies which data to pull. The number 500 specifies which row to pull from. Put together, we are telling R to print the title from the gayguides found on row 500. 
...
max(BostonWomenVoters$Age)
min(BostonWomenVoters$Age, na.rm = TRUE)
max(BostonWomenVoters$Age, na.rm = TRUE)
max(BostonWomenVoters$Age[500:510], na.rm = TRUE)
```

(@) What did you learn about the dataset? What can you tell me about it? 

> I analyzed the BostonWomenVoters dataset. There are 34 variables and 10702 observations. The variables include things like ward number, page number, precinct, name, age, country of birth, occupation, husband name, father name, etc. I was also able to figure out that the oldest woman listed was 91 and the youngest was 20. To find this information, I instructed R to ignore any values that were NA!  
