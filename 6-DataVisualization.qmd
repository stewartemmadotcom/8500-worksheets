...
> Gapminder is a data set that includes different countries in different years along with demographic information about their life expectancy, population, and GDP per capita. 
...
```{r}
p <- ggplot(date = gapminder) # here, I am reading the data from gapminder to ggplot 
p <- ggplot(data = gapminder, mapping = aes(x = pop, y = lifeExp)) # next, I tell ggplot what I want to map to the x and y axes 
p + geom_point() # finally, I tell ggplot what shape/form I want the graph to be in 
```
...
ggplot(data = rec, mapping = aes(x = population, y = total_expenditures, color=city)) + geom_point() 
```
...
```{r}
ggplot(BostonWomenVoters, aes(x = Age)) + geom_histogram(bins=71, color="black", fill="lightblue")
```

(@) Explain, what does changing the number of bins do? 

> Changing the number of bins compresses or stretches out the width of the bars on the graph. I used 71 bins because that is about the age range of voters, so each bar basically represents one age. If I wanted to represent age ranges, I would have chosen a smaller number of bins. 
...
```{r}
data(gayguides)
gg_south <- gayguides %>% filter(state == "SC" | state == "GA" | state == "NC")
south1980 <- gg_south %>% filter(Year == 1980) %>% group_by(state) %>% summarize(count = n())

ggplot(data = south1980, aes(x = state, y = count)) + geom_col()
```

(@) Using the `gayguides` data, plot the number of religious institutions in each state.
```{r}
unique(gayguides$type)
grepl("Religious", gayguides$type)
religious_gg <- gayguides %>% filter(grepl("Religious", gayguides$type))

religious_bystate <- religious_gg %>% group_by(state) %>% summarize(count = n())

ggplot(data = religious_bystate, aes(x=state, y=count)) + geom_col()
```
...
> Based on the gayguides data frame, what was the distribution of locations that catered to women across the states? 

(@) Now write the code to answer that question and use comments (`#`) to explain your thinking as you go.
```{r}
women.locs <- gayguides %>% 
  filter(grepl("G|L", gayguides$amenityfeatures)) # here, I am filtering the gayguides dataset to identify that locations that catered to women
  %>% group_by(state)  # next, I sort the data by state
  %>% summarize(count = n()) # and summarize with a column called count

ggplot(data=women.locs, aes(x=state, y=count)) + geom_col() # finally, I use the subsetted data to create a bar chart with states on the X axis and the number of locations catered to women on the Y axis. 

# As expected, CA and NY have the highest number of locations catered to women; this does not tell me much because those states had the highest number of locations over all. Maybe it would be more interesting if I graphed the percentage of locations in each state that catered to women... 
...
```{r}
religion.byyear <- gayguides %>% filter(grepl("Religious", gayguides$type)) %>% group_by(Year) %>% summarize(count = n())
ggplot(data=religion.byyear, aes(x=Year, y=count)) + geom_line()
```
...
```{r}
risk.byyear <- gayguides %>% filter(grepl("HOT", gayguides$amenityfeatures) | grepl("AYOR", gayguides$amenityfeatures) | grepl("inquire locally", gayguides$description, ignore.case=TRUE) | grepl("inquire locally", gayguides$streetaddress)) %>% group_by(Year) %>% summarize(count = n())

ggplot(data=risk.byyear, aes(x=Year, y=count)) + geom_line()
...
```{r}
str(recreational.data)
sc.rec <- recreational.data %>% filter(state == "SC")
...
```{r}
unique(gayguides$type)
ggtypes <- gayguides %>% filter(type == "Cruising Areas" | type == "Book Store" | type == "Restaurant" | type == "Bars/Clubs" | type == "Hotel") %>% filter(Year >= 1965 & Year <= 1980) %>% group_by(type, Year) %>% summarize(count = n())

ggplot(ggtypes, aes(x=Year, y=count)) + geom_line() + facet_wrap(~type)
```
...
str(rec.workers)
unique(rec.workers$type_of_worker)
avgworkers <- rec.workers %>% pivot_longer(!city:type_of_worker, names_to="year", values_to="count") %>% group_by(type_of_worker, year) %>% summarize(count = n())

ggplot(avgworkers, aes(x=year, y=count)) + geom_col() + facet_wrap(~type_of_worker)
```
...
```{r}
women.locs <- gayguides %>% 
  filter(grepl("G|L", gayguides$amenityfeatures)) %>% 
  group_by(Year) %>%
  summarize(count = n())
total.locs <- gayguides %>% 
  group_by(Year) %>%
  summarize(count = n())

plot1 <- ggplot(women.locs, aes(x=Year, y=count)) + geom_line() + scale_y_continuous(limits=c(0, 5000))

plot2 <- ggplot(total.locs, aes(x=Year, y=count)) + geom_line() + scale_y_continuous(limits=c(0,5000))
...
(@) Plot 1 explanation: 
> I plotted the locations that catered to women over the course of the time period of the data set. I decided to also set this plot next to one that showed the total number of locations over time so as to contextualize the first graph. Here, you can really visualize how male-dominated the Bob Damron guides were. 
...
women.locs <- gayguides %>% 
  ...
  group_by(Year) %>%
  summarize(count = n()) %>% 
  mutate(percent_increase = count - lag(count)/lag(count)) * 100
ggplot(women.locs, aes(x=Year, y=percent_increase)) + geom_col()
```

(@) Plot 2 explanation: 
>Here, I also mapped women locations; however, I used a bar chart and a simple equation to map the percent increase/decrease each year of the number of locations.
...
```{r}
plot1 <- ggplot(women.locs, aes(x=Year, y=count)) + geom_area() + scale_y_continuous(limits=c(0, 5000))

plot2 <- ggplot(total.locs, aes(x=Year, y=count)) + geom_area() + scale_y_continuous(limits=c(0,5000))
...
(@) Plot 3 explanation: 
>Here, I again graphed the locations that catered to women and the total locations. This time, I used an area chart, which I think helps readers process the information more quickly. Additionally, I decided to stack the plots rather than put them side to side so that it is more clear these are not continuous with one another and are plotting separate things. 
