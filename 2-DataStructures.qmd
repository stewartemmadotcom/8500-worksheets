...
>By using a negative, this code informed R to exclude the numbers in positions 1 and 3. 
...
>First, we assigned logical values to each variable in the vector. When that code was entered, only the true values were printed. Then, we used the code to ask R to print out all the values that are greater than 7. 
...
(@) What happened here? Explain this in your own words. 
  Here, we assigned names to each element. The names were letters, and we only assigned them to the first four elements. Then, we queried R to print the element we named "d," and it returned the fourth element, 8.35. 
...
>In the first example, we tell R to print a range of columns, from 2-3. Because we do not specify the rows, it prints all the rows. In the second example, we tell R to print rows 1-3 but don't specify which columns. Therefore, we get the first three rows and all the columns. 
...
head(BostonWomenVoters[,16:17])

```

> Here, I queried to find only the Occupation and Place of Business of the first five women in the dataset to get a sense of the kinds of work they were doing and where. 
...
```{r}
ca.locations <- gayguides$state=="CA"
library(DigitalMethodsData)
data(gayguides)

gg_70s <- gayguides[gayguides$Year>=1970 & gayguides$Year<=1979,]
print(gg_70s)
gg_ca_70s <- gg1970s[gg1970s$state=="CA",]
#this works 
print(gg_ca_70s)

#but this could also work 
gg_70s <- gayguides$Year>=1970 & gayguides$Year<=1979
gg_ca <- gayguides$state=="CA"
ca70s <- gg_ca & gg_70s
cali70s <- gayguides[ca70s,]

#or this 
alsocali70s <- gayguides[gayguides$state=="CA" & gayguides$Year>=1970 & gayguides$Year<=1979,]
...
womenlocs <- grep("G", gayguides$amenityfeatures, ignore.case = TRUE)
gg.women <- gayguides[womenlocs,]
print(gg.women)
t```
...
```{r}
str(BostonWomenVoters)
over.50 <- BostonWomenVoters$Age>50
under.50 <- BostonWomenVoters$Age<50
nrow(BostonWomenVoters[over.50,])
nrow(BostonWomenVoters[under.50,])
#There were significantly more women voters under the age of 50, at least included in this data set. 
```
...
```{r}
mainst <- grep("Main", BostonWomenVoters$Street.of.Residence.on.April.1)
bwv.mainst <- BostonWomenVoters[mainst,]
```
...
age.order <- order(BostonWomenVoters$Age, decreasing = TRUE)
by.age <- BostonWomenVoters[age.order,]
eldest <- head(by.age)
eldest$Occupation
```
