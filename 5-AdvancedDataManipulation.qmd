...
> I want to compare the number of women and men of different races/ethnicities who were judges.
```{r}
data(judges)
str(judges)
```
...
```{r}
judges.g <- judges %>% 
  select(Last.Name, First.Name, Gender, Race.or.Ethnicity) %>% 
...
> How many locations were there over the years in OKC that were woman-friendly? 
> Total of 53 
```{r}
w.gg.okc <- gayguides %>% 
  filter(state == "OK", city == "Oklahoma City") %>% 
...
```{r}
str(gayguides)
ttsy <- function(gyear, gstate) {
  data(gayguides)

  gayguides %>%
  filter(state == gstate, Year == gyear) %>%
...
```{r}
BWV.Occ <- BostonWomenVoters %>%
  mutate(
    occupation_category = fct_collapse(Occupation,
      "Professional" = c("Teacher", "Nurse", "Social worker", "Physician", "School Teacher", "School teacher", "Teaching", "Librarian"),
      "Clerical" = c("Clerk", "Stenographer", "General clerk", "Secretary", "Bookkeeper", "Invoice Clerk", "stenographer"),
      "Domestic" = c("Housewife", "At Home", "At home", "Housekeeper", "Married, Housewife", "Married, housewife", "housewife")
    ),
    # Order by typical class hierarchy
    occupation_category = fct_relevel(occupation_category, 
                                     "Professional", "Clerical", "Domestic")
...
BWV.Occ2 <- BWV.Occ %>% 
  group_by(Ward, occupation_category, na.rm = TRUE) %>%
  summarize(
    count = n(),
    avg_age = mean(Age, na.rm=TRUE),
    min_age = min(Age, na.rm=TRUE),
    max_age = max(Age, na.rm=TRUE)) %>%
  arrange(desc(count), occupation_category)

```

(@) What did you discover? Write 2-3 sentences about what this tells us about Boston women voters.

> The largest category by far is Domestic workers, whose average age differs based on ward but is somewhere between 39 and 47. The oldest domestic is 91 and the youngest is 20. The average age among clerical workers, the second largest occupation category, was between 34 and 27. For professionals, the smallest category, was around 32. This makes historical sense, as younger women likely had more educational and career opportunities. 
