...
> First, I decided that the best way to figure out the max year value would be with the max() function. Then, I used that function inside the if statement to test, specifically, the year column of the data set. 
...
> This works because length specifies the entire number of items in the variable presidents; 1:length means to start from the beginning and go through each item. Instead of i, p was used this time. 
...
```{r}
spring.classes <- c("Digital Methods I", "Theories of Digital Analaysis", "Local History and Digital Projects") 
for (c in 1:length(spring.classes)) {
  print(paste("I am enrolled in", spring.classes[c]))
}
...
> Here, we used "next" to skip values less than 3. Then, we used "break" to end the loop if it came across a value greater than or equal to 5. 
...
```{r}
gvil <- gayguides$city=="Greenville"
gvilgg <- gayguides[gvil,]
...
```{r}
historyStudents("Bean and Jack", "Dog Training School")
```
...
charcount <- function(string) {
  thecount <- nchar(string)
  print(thecount)
}

charcount("bean and jack")

```

(@) Reflect on the process of building the above function. How did you go about figuring this out? 

> For this, I first needed to figure out what function counts the number of characters in a string. I found the nchar function through a quick google search on the GeeksforGeeks.com website. Then, I used that function to create my own function, just as in the examples above. 
...
> Rather than being printed, it is stored to a variable called GeorgeWashington! 
...
```{r}
state.year <- function(gstate, gyear) {
  data(gayguides)

  for (i in 1:nrow(gayguides)) {
    if (gayguides$state[i] == gstate && gayguides$Year[i] == gyear) {
...
bwv.housewife <- bwv.occupation("Housewife")
...
count_by_state <- function (State) {
  ...
  print(paste("There are", state.count, "locations in", State))
}
...
```{r}
state.count <- 0
count_by_state <- function (State, Year) {
  data(gayguides)

  for (i in 1:nrow(gayguides)) {
    if (gayguides$state[i] == State && gayguides$Year[i] == Year) {
      state.count <- state.count + 1 
    }
  }
  print(paste("In", Year, "there were", state.count, "locations in", State))
}

count_by_state("AR", 1980)
```
...
> Loops and conditionals go hand in hand. For querying a data set, using if and for statements is helpful for finding specific information and returning it in a specific format. Functions make queries more convenient. Rather, than rewriting a loop over and over for different queries, functions make it easy to save a loop for later use. 
